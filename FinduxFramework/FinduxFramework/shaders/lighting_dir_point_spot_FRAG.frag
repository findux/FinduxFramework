...
{ 
    ...
    // Ambient ----------------------------------------------------------------------------------
	vec3 ambient = spotLight.ambient * material.ambient * vec3(texture(material.diffuseMap, frag.TexCoord));
	vec3 outColor = vec3(0.0f);	

	outColor += calcDirectionalLightColor(sunLight, normal, viewDir);

   for(int i = 0; i < MAX_POINT_LIGHTS; i++)
        outColor += calcPointLightColor(pointLights[i], normal, frag.FragPos, viewDir);  

	// If the light isn't on then just return 0 for diffuse and specular colors
	if (spotLight.on == 1)
		outColor += calcSpotLightColor(spotLight, normal, frag.FragPos, viewDir);

	vec4 rayMatLoc = rayMat * vec4( frag.FragPos,1.0f);


	if (fuzzyCompare(frag.intersected.x,1))
	{
		frag_color = vec4(ambient + outColor, 1.0f) + vec4(1.0f, 0.0f, 0.0f, 0.05f);
	}
	else
	{
...
vec3 calcDirectionalLightColor(DirectionalLight light, vec3 normal, vec3 viewDir)
{
    ...
    vec3 diffuse = light.diffuse * NdotL * vec3(texture(material.diffuseMap, frag.TexCoord));
...
vec3 calcSpotLightColor(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
	...
	// Diffuse ----------------------------------------------------------------------------------
    float NdotL = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = spotLight.diffuse * NdotL * vec3(texture(material.diffuseMap, frag.TexCoord));
